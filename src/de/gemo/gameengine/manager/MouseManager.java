...
public class MouseManager {
	...
	public void update() {
		...
		// catch MouseMovement. NOTE: This can only be done ONCE, that's why we
		// do it here
		dX = Mouse.getDX();
		dY = -Mouse.getDY();
		...
		if (wheel != 0) {
			engine.handleMouseWheel(new MouseWheelEvent(correctedX, correctedY, wheel > 0));
		}
		...
		for (int index = 0; index < 5; index++) {
			...
			if (!currentState && oldState) {
				// throw MouseUpEvent
				engine.handleMouseUp(new MouseReleaseEvent(correctedX, correctedY, MouseButton.byID(index)));
				...
				engine.handleMouseDown(new MouseClickEvent(correctedX, correctedY, MouseButton.byID(index), doubleclick));
		...
		if (!Mouse.isGrabbed()) {
			if (currentX != correctedX || currentY != correctedY) {
				...
				engine.handleMouseMove(new MouseMoveEvent(correctedX, correctedY, correctedDX, correctedDY));
			}
			currentX = correctedX;
			currentY = correctedY;
		}
