 ...
 * @author kevin
 */
public class RectangleTextureLoader {
    /**
     * Load a texture with a given format from the supplied input stream
     * 
     * @param format
     *            The format of the texture to be loaded (something like "PNG" or "TGA")
     * @param in
    ...
    public static Texture getTexture(String format, InputStream in, boolean flipped, int filter) throws IOException {
        return RectangleTextureLoaderImpl.get().getTexture(in, in.toString() + "." + format, flipped, filter);
    }
}
