package de.gemo.gameengine.textures.loader;
...
public class RectangleTextureLoaderImpl {
	/** The renderer to use for all GL operations */
	protected static SGL GL = Renderer.get();
	/** The standard texture loaded used everywhere */
	private static final RectangleTextureLoaderImpl loader = new RectangleTextureLoaderImpl();
	 ...
	 */
	public static RectangleTextureLoaderImpl get() {
		return loader;
	}
	 ...
	 */
	private RectangleTextureLoaderImpl() {
	}
	...
	public RectangleTexture getTexture(InputStream in, String resourceName, boolean flipped, int filter, int[] transparent) throws IOException {
		if (deferred) {
			return new DeferredRectangleTexture(in, resourceName, flipped, filter, transparent);
		}
		...
		if (holdTextureData) {
			RectangleTexture tex = (RectangleTexture) hash.get(resName);
			if (tex != null) {
				return tex;
			}
		} else {
			...
			if (ref != null) {
				RectangleTexture tex = (RectangleTexture) ref.get();
				if (tex != null) {
					return tex;
				} else {
					hash.remove(resName);
		...
		RectangleTexture tex = getTexture(in, resourceName, GL_TEXTURE_RECTANGLE_ARB, filter, filter, flipped, transparent);
	 ...
	 *             Indicates a failure to load the image
	 */
	private RectangleTexture getTexture(InputStream in, String resourceName, int target, int magFilter, int minFilter, boolean flipped, int[] transparent) throws IOException {
		...
		RectangleTexture texture = new RectangleTexture(resourceName, target, textureID);
		// bind this texture
		...
		GL.glTexImage2D(target, 0, dstPixelFormat, width, height, 0, srcPixelFormat, SGL.GL_UNSIGNED_BYTE, textureBuffer);
	...
	public Texture getTexture(ImageData dataSource, int filter) throws IOException {
		...
		// create the texture ID for this texture
		int textureID = createTextureID();
		RectangleTexture texture = new RectangleTexture("generated:" + dataSource, target, textureID);
	...
	public void reload() {
		...
		while (texs.hasNext()) {
			((RectangleTexture) texs.next()).reload();
		}
	 ...
	 */
	public int reload(RectangleTexture texture, int srcPixelFormat, int componentCount, int minFilter, int magFilter, ByteBuffer textureBuffer) {
		int target = GL_TEXTURE_RECTANGLE_ARB;
		int textureID = createTextureID();
		GL.glBindTexture(target, textureID);
