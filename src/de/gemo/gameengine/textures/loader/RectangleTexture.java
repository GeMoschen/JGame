...
/**
 * A texture to be bound within JOGL. This object is responsible for keeping track of a given OpenGL texture and for calculating the texturing mapping coordinates of the full image.
 * 
 ...
 * @author Brian Matzon
 */
public class RectangleTexture implements Texture {
     ...
     *            The GL texture ID
     */
    public RectangleTexture(String ref, int target, int textureID) {
        this.target = target;
        this.ref = ref;
    ...
    public static void bindNone() {
        lastBind = null;
        GL.glDisable(GL_TEXTURE_RECTANGLE_ARB);
    }
    ...
    public void release() {
        ...
        if (cacheName != null) {
            RectangleTextureLoaderImpl.get().clear(cacheName);
        } else {
            RectangleTextureLoaderImpl.get().clear(ref);
        }
    }
    ...
    public byte[] getTextureData() {
        ...
        bind();
        GL.glGetTexImage(GL_TEXTURE_RECTANGLE_ARB, 0, hasAlpha() ? SGL.GL_RGBA : SGL.GL_RGB, SGL.GL_UNSIGNED_BYTE, buffer);
    ...
    private class ReloadData {
        ...
        public int reload() {
            ...
            return RectangleTextureLoaderImpl.get().reload(RectangleTexture.this, srcPixelFormat, componentCount, minFilter, magFilter, textureBuffer);
        }
    }
}