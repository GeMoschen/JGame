...
public class Hook extends EntityCollidable {
	...
	public Hook(Player player, float x, float y, float angle, boolean left) {
		...
		shape.m_radius = halfWidth / Physics2D.PX_PER_M;
	...
	public void createJoint() {
		if (this.hookPos != null && this.joint == null && this.hookAnchor != null) {
			...
			Physics2D._world.createJoint(this.joint);
		}
	}
	...
	public void update(int delta) {
		...
		if (this.hookPos != null) {
			...
			if (this.hookAnchor == null) {
				...
				hookAnchor = Physics2D._world.createBody(bodyDef);
			}
	...
	public void debugRender() {
		...
		glPushMatrix();
		{
			...
			glTranslatef(pos.x * Physics2D.PX_PER_M, pos.y * Physics2D.PX_PER_M, 9);
			Color.yellow.bind();
	...
	public void destroyBody() {
		super.destroyBody();
		if (this.hookAnchor != null) {
			Physics2D._world.destroyBody(this.hookAnchor);
		}
	}
	...
	public boolean beginCollision(EntityCollidable entity, Contact contact) {
		if (entity instanceof Ground) {
			...
			int yPos = (int) (contact.getManifold().localPoint.y * 10 * Physics2D.PX_PER_M);
			int yPos2 = (int) (groundBox.getHalfHeight() * 10);
			boolean isDown = groundBox.isDown() && (yPos == yPos2 || yPos == yPos2 - 1 || yPos == yPos2 + 1);
