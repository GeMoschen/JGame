...
public class Player extends EntityCollidable {
	...
	public Player(float x, float y) {
		try {
			...
			// animation right
			{
				...
				_animationRight = multiTexture.toAnimation();
			}
			// animation left
			{
				final MultiTexture multiTexture = new MultiTexture(width, height);
				for (int dx = 0; dx < 8; dx++) {
					multiTexture.addTextures(singleTexture.crop(7 * width - width * dx, height * 1, width, height));
				}
				_animationLeft = multiTexture.toAnimation();
			}
	...
	private void updateAnimation() {
		...
		if (onGround && Math.abs(getLinearVelocity().x) > 1) {
			_currentAnimation.step(Math.max(10 * (Math.abs(getLinearVelocity().x) / 10f), 1));
		} else {
			if (!onGround) {
				if (hookCatched) {
					_currentAnimation.goToFrame(5);
				} else {
					if(isJumping) {
						_currentAnimation.goToFrame(1);
					} else {
						_currentAnimation.goToFrame(7);
					}
				}
			} else {
...
//		this.debugRender();
		...
		glPushMatrix();
		{
			Vec2 pos = this.getPosition();
			// translate to _center
			glTranslatef(pos.x * Physics2D.pxPerM, (pos.y - 0.15f) * Physics2D.pxPerM, 10);
