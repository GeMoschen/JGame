...
public class CollisionHelper {
    ...
    public Vector MinimumTranslationVector = null;
    ...
    public static boolean isVectorInHitbox(EasyVector vector, ComplexHitbox hitbox) {
    ...
    // Jordan Curve Theorem - very fast
    public static boolean isVectorInHitbox(ComplexVector vector, ComplexHitbox hitbox) {
        int nvert = hitbox.getPointCount();
        int i, j, count = 0;
        for (i = 0, j = nvert - 1; i < nvert; j = i++) {
            if (((hitbox.getPoint(i).getY() > vector.getY()) != (hitbox.getPoint(j).getY() > vector.getY())) && (vector.getX() < (hitbox.getPoint(j).getX() - hitbox.getPoint(i).getX()) * (vector.getY() - hitbox.getPoint(i).getY()) / (hitbox.getPoint(j).getY() - hitbox.getPoint(i).getY()) + hitbox.getPoint(i).getX())) {
    ...
    public static boolean isColliding(ComplexHitbox hitboxA, ComplexHitbox hitboxB) {
        for (ComplexVector vector : hitboxA.getPoints()) {
            if (isVectorInHitbox(vector, hitboxB)) {
                return true;
            }
        }
        for (ComplexVector vector : hitboxB.getPoints()) {
    ...
    // find intersections
    public static List<EasyVector> findIntersection(ComplexHitbox hitboxA, ComplexHitbox hitboxB) {
        List<EasyVector> result = new ArrayList<EasyVector>();

        ComplexVector a1 = null, a2 = null;
        ComplexVector b1 = null, b2 = null;
        for (int i = 0; i < hitboxA.getPointCount(); i++) {
            ...
            for (int j = 0; j < hitboxB.getPointCount(); j++) {
                ...
                EasyVector vector = findIntersection(a1, a2, b1, b2);
                if (vector != null) {
                    result.add(vector);
                }
            }
    ...
    public static EasyVector findIntersection(ComplexVector start1, ComplexVector end1, ComplexVector start2, ComplexVector end2) {
        double denom = ((end1.getX() - start1.getX()) * (end2.getY() - start2.getY())) - ((end1.getY() - start1.getY()) * (end2.getX() - start2.getX()));
        ...
        double numer = ((start1.getY() - start2.getY()) * (end2.getX() - start2.getX())) - ((start1.getX() - start2.getX()) * (end2.getY() - start2.getY()));
        double r = numer / denom;

        double numer2 = ((start1.getY() - start2.getY()) * (end1.getX() - start1.getX())) - ((start1.getX() - start2.getX()) * (end1.getY() - start1.getY()));
        ...
        return new EasyVector((int) (start1.getX() + (r * (end1.getX() - start1.getX()))), (int) (start1.getY() + (r * (end1.getY() - start1.getY()))));
    }
    ...
    public static Vector findIntersection(Vector start1, Vector end1, Vector start2, Vector end2) {
        ...
        // Find intersection point
        return new Vector(start1.getX() + (r * (end1.getX() - start1.getX())), start1.getY() + (r * (end1.getY() - start1.getY())));
    }
}
